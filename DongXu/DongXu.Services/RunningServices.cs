...
namespace DongXu.Services
{
    ...
    public class RunningServices : IRunning
    {
        ...
        public List<TargetDetails> GetRunnings()
        {
            ...
            {
                conn.Open();
                string sql = @"select b.name,a.numberids,a.per from (select statusid,count(1) as numberids,TRUNC(count(1)/(select count(1)  from targetdetails a inner join target b on a.targetid=b.id)*100,2) as per from targetdetails group by statusid)  a inner join status b on a.statusid = b.id";
